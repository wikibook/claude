# 2장 클로드와 대화하는 기술

> **책 2장** 독자를 위한 보충 자료입니다.
> 클로드 기능 변화에 맞춰 지속적으로 업데이트합니다.

## 이 장에서 받을 수 있는 것

2장은 파일을 내려받아 실행하기보다, 프롬프트를 어떻게 쓰고 고치는지 익히는 장입니다.
책 예제 2.1~2.20의 [IN] 요청문을 그대로 모아 두었습니다.

| 자료 | 내용 | 책 연결 |
| --- | --- | --- |
| [`프롬프트/예제-프롬프트.md`](프롬프트/예제-프롬프트.md) | 예제 2.1~2.20 전체 [IN] 요청문 | 2.1~2.20절 |
| [`샘플/고객문의-샘플.txt`](샘플/고객문의-샘플.txt) | 프로젝트 컨텍스트 실습용 고객 문의 | 2.4절 예제 2.12 |
| [프롬프트 템플릿 모음](../../resources/프롬프트-템플릿.md) | 2장·다른 장에서 자주 쓰는 요청 요약 | 2장 이후 |
| [FAQ·문제 해결](../../resources/faq-문제해결.md) | 답변 품질이 낮거나 파일 처리가 막힐 때 | 2장 이후 |

## 추천 자료

| 자료 | URL | 설명 |
| --- | --- | --- |
| Claude 101 | https://anthropic.skilljar.com/claude-101 | 앤트로픽 공식 입문 강의 — 핵심 기능·일상 업무 활용 |
| AI Fluency: Framework & Foundations | https://anthropic.skilljar.com/ai-fluency-framework-foundations | AI와 효과적·윤리적으로 협업하는 기초 |
| coreyhaines31/marketingskills | https://github.com/coreyhaines31/marketingskills | 마케팅 전략·카피·CRO 스킬 40+. 예제 2.1·2.11 마케팅 프롬프트 심화 |
| anthropics/prompt-eng-interactive-tutorial | https://github.com/anthropics/prompt-eng-interactive-tutorial | 프롬프트 엔지니어링 대화형 튜토리얼 |
| dair-ai/Prompt-Engineering-Guide | https://github.com/dair-ai/Prompt-Engineering-Guide | LLM 공통 프롬프트 가이드 |
| anthropics/courses | https://github.com/anthropics/courses | API·프롬프트 엔지니어링 강의 자료 |
| f/awesome-chatgpt-prompts | https://github.com/f/awesome-chatgpt-prompts | 역할·Few-shot 참고 예시 모음 |
| langgptai/awesome-claude-prompts | https://github.com/langgptai/awesome-claude-prompts | 클로드용 프롬프트 예시 |

## 공식 문서

| 문서 | URL | 설명 |
| --- | --- | --- |
| 프롬프트 엔지니어링 가이드 | https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview | 프롬프트 작성 원칙과 실전 패턴 |
| 클로드 지원 센터 | https://support.claude.com | 프로젝트, 대화, 파일 첨부 등 사용법 |