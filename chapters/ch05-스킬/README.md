...
책 5장에서 만든 스킬을 바로 내려받아 쓸 수 있도록 정리했습니다.
각 폴더의 `SKILL.md`는 클로드 공식 스킬 형식(YAML 머리말 + 본문)을 따릅니다.
폴더를 `.zip`으로 압축해 클로드 스킬 업로드 메뉴에 등록하면 됩니다(책 5.2.2절).

| 폴더 | 내용 | 책 연결 |
| --- | --- | --- |
| [`메모-할일-정리/`](메모-할일-정리/SKILL.md) | 흩어진 메모를 마크다운 체크리스트로 정리 | 5.3절 예제 5.2~5.5 |
| [`학습-보조/`](학습-보조/SKILL.md) | 비유 설명 → 배경 맞춤 예시 → 퀴즈로 이해도 확인 | 5.4절 예제 5.6~5.9 |
...
`skills/` 폴더에는 예시 스킬이 네 도메인으로 묶여 있습니다(책 5.2.2절).
...
| knowledge-work-plugins | https://github.com/anthropics/knowledge-work-plugins | 생산성·세일즈·마케팅·법률·재무 등 업무 영역별 플러그인 묶음입니다(설치는 7장). |

> **확인 습관** — 커뮤니티 스킬은 설치 전에 `SKILL.md`를 열어 어떤 동작을 지시하는지 직접 읽어보세요.
> 최근 업데이트(6개월 이내), 설명 문서(README)의 충실함도 함께 확인하면 좋습니다(책 5.2.2절).
